...
<section class="bg-white mt-10 p-10 rounded-lg">
    <form action="/daftar-penerbit/{{ $penerbit->id }}" method="POST" class="all_form flex gap-12">
        @csrf
        @method('PUT')
        ...
        <div class="w-1/2 flex justify-center items-start pt-4">
            <div class="w-[350px] flex flex-col items-center text-center">
                ...
                <p class="text-[#35094D] font-bold text-lg">Edit Penerbit</p>
                <p class="text-gray-400 text-sm mt-1">Perbarui data supplier / penerbit</p>

                <div class="mt-6 w-full bg-[#35094d08] rounded-xl p-4 text-left">
                    <p class="text-xs text-gray-400 font-semibold uppercase mb-2">Data Saat Ini</p>
                    <p class="text-sm font-bold text-[#35094D]">{{ $penerbit->nama_penerbit }}</p>
                    @if($penerbit->bukus_count ?? false)
                        <p class="text-xs text-gray-400 mt-1">{{ $penerbit->bukus_count }} judul buku terdaftar</p>
                    @endif
                </div>
            </div>
        </div>
