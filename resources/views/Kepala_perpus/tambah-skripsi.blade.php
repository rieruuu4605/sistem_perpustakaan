...
<section class="bg-white mt-10 p-10 rounded-lg">
    <form action="/koleksi-skripsi/simpan" method="POST" class="all_form flex gap-12">
        @csrf
        ...
        <div class="w-1/2 flex justify-center items-start pt-4">
            <div class="w-[350px] flex flex-col items-center text-center">
                ...
                <p class="text-[#35094D] font-bold text-lg">Input Skripsi</p>
                <p class="text-gray-400 text-sm mt-1">Tugas Akhir / Skripsi Mahasiswa</p>

                <div class="mt-8 w-full text-left space-y-2">
                    <p class="text-xs text-gray-400 font-semibold uppercase">Panduan Pengisian</p>
                    ...
                    <p class="text-xs text-gray-500">• Abstrak minimal menjelaskan isi skripsi</p>
                </div>
            </div>
        </div>
        ...
        <div class="w-1/2 space-y-4">
                    ...
                    <div class="flex flex-col gap-1 w-1/2">
                        ...
                        <select name="nomor_rak"
                            ...
                            @foreach(range('A', 'Z') as $huruf)
                                <option value="{{ $huruf }}" {{ old('nomor_rak') == $huruf ? 'selected' : '' }}>
                                    Rak {{ $huruf }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col gap-1 w-1/2">
                        ...
                        <select name="nomor_baris"
                            ...
                            @foreach(range(1, 4) as $angka)
                                <option value="{{ $angka }}" {{ old('nomor_baris') == $angka ? 'selected' : '' }}>
                                    Baris {{ $angka }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div>
                ...
                <textarea name="abstrak" rows="4" placeholder="Ringkasan isi skripsi..."
                    class="w-full border rounded-md px-4 py-2 mt-1 border-gray-300 text-gray-400">{{ old('abstrak') }}</textarea>
            </div>
            ...
            <div class="flex gap-4 pt-4">
                ...
                <button type="submit"
                    ...
                    <span class="text_simpan">Simpan Skripsi</span>
                </button>
            </div>
        </div>
    </form>
